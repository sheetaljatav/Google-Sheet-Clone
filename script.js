...
function evaluateFormula(formula) {
    ...
    if (sumRegex.test(formula)) {
        ...
        const range = formula.match(maxRegex)[1];
        return calculateMax(range);
    } else if (minRegex.test(formula)) {
        const range = formula.match(minRegex)[1];
        return calculateMin(range);
    } else if (countRegex.test(formula)) {
        const range = formula.match(countRegex)[1];
        return calculateCount(range);
    } else {
        return eval(formula);
    }
}
...
function calculateAverage(range) {
    const cells = getCellsInRange(range);
    const sum = cells.reduce((sum, cell) => sum + parseFloat(cell.value || 0), 0);
...
function calculateCount(range) {
    const cells = getCellsInRange(range);
    return cells.filter(cell => !isNaN(parseFloat(cell.value))).length;
}
...
function getCellsInRange(range) {
    ...
    for (let row = startRow; row <= endRow; row++) {
        for (let col = startCol; col <= endCol; col++) {
            const cell = document.querySelector(`input[data-row="${row}"][data-col="${String.fromCharCode(64 + col)}"]`);
            if (cell) {
                cells.push(cell);
            }
        }
...
function addRow() {
    ...
    const rowCount = spreadsheet.childElementCount / 27;
    for (let col = 0; col <= 26; col++) {
        ...
        if (col === 0) {
            ...
            input.dataset.col = String.fromCharCode(64 + col);
...
function addColumn() {
    const spreadsheet = document.getElementById('spreadsheet');
    const colCount = spreadsheet.childElementCount / 101;
    for (let row = 0; row <= 100; row++) {
        const cell = document.createElement('div');
        cell.className = 'cell';
        if (row === 0) {
            cell.className += ' header';
            cell.textContent = String.fromCharCode(64 + colCount);
        } else {
            const input = document.createElement('input');
            input.type = 'text';
            input.dataset.row = row;
            input.dataset.col = String.fromCharCode(64 + colCount);
            input.addEventListener('input', handleInput);
            input.addEventListener('focus', () => selectedCell = input);
            cell.appendChild(input);
        }
